...
class EmotionAnalysisRequest(BaseModel):
    ...
    include_explanation: bool = Field(True, description="Include AI explanation of the analysis")
    threshold: float = Field(0.5, ge=0.0, le=1.0, description="Confidence threshold for emotion detection")
...
class EmotionAnalysisResponse(BaseModel):
    ...
    # Core results
    emotions: dict[str, float] = Field(..., description="All emotions with confidence scores")
    primary_emotion: str = Field(..., description="Highest confidence emotion")
    primary_confidence: float = Field(..., ge=0.0, le=1.0)
    secondary_emotions: list[str] = Field(default_factory=list, description="Other detected emotions")
    detected_emotions: list[str] = Field(..., description="All emotions above threshold")

    # Analysis metadata
    confidence_level: str = Field(..., description="Overall confidence: high/medium/low")
    emotional_complexity: float = Field(..., ge=0.0, le=1.0, description="Measure of emotion diversity")

    # Explainability
    explanation: Optional[str] = Field(None, description="Natural language explanation")
    word_importance: Optional[dict[str, float]] = Field(None, description="Word-level importance scores")
...
class EmotionBatchRequest(BaseModel):
    ...
    texts: list[str] = Field(..., min_length=1, max_length=10, description="List of texts to analyze")
