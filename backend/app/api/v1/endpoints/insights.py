...
async def get_mood_patterns(
    ...
    for period, emotions in hour_emotions.items():
        ...
        dominant = counter.most_common(1)[0] if counter else None
        if dominant:
            period_insights[period] = {"emotion": dominant[0], "count": dominant[1]}
...
async def get_music_taste_profile(
    ...
    good_recs = [
        r for r in recommendations if r.feedback_rating and r.feedback_rating >= 4
    ]
    bad_recs = [
        r for r in recommendations if r.feedback_rating and r.feedback_rating <= 2
    ]
    ...
    profile = {
        ...
        "music_emotion_preferences": [
            {"emotion": e, "count": c} for e, c in top_music_emotions
        ],
        "preferred_emotions": [
            {"emotion": e, "count": c} for e, c in preferred_emotions.most_common(3)
        ],
        "less_preferred_emotions": [
            {"emotion": e, "count": c} for e, c in avoided_emotions.most_common(3)
        ],
    ...
    if top_music_emotions:
        ...
        profile[
            "description"
        ] += f"You tend to seek out music that evokes **{top_emotion}**."

    return {"has_data": True, "profile": profile}
