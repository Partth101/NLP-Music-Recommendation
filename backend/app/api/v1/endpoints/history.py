...
async def get_history_stats(
    ...
    most_common = [{"emotion": e, "count": c} for e, c in emotion_counts.most_common(5)]
    ...
    favorite_artists = [
        {"artist": a, "count": c} for a, c in artist_counts.most_common(5)
    ]
    ...
    while current_date <= end_date:
        day_analyses = [a for a in analyses if a.created_at.date() == current_date]

        if day_analyses:
            # Get average confidence and primary emotions for the day
            avg_confidence = sum(a.primary_confidence for a in day_analyses) / len(
                day_analyses
            )
            day_emotions = Counter(a.primary_emotion for a in day_analyses)
            dominant_emotion = (
                day_emotions.most_common(1)[0][0] if day_emotions else None
            )
